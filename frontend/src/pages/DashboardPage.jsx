...
import { X, AlertTriangle, ChevronRight, BookOpen, LogOut, Trash, CheckCircle, RotateCcw } from 'lucide-react'; // Added RotateCcw for "Start Again"
...
// ADDED onMarkActive prop
const CourseCard = ({ course, onMarkComplete, onDelete, isActive, onMarkActive }) => {
    const statusColor = isActive ? sageGreen.lightHover : '#9ca3af';
    const statusBg = isActive ? '#3b82f620' : '#4b5563';
    
    // Determine the border color for the card's initial state for consistency
    const initialBorderLeftColor = 'transparent';
    ...
    return (
        <Link
            ...
            style={{
                ...
                ...styles.tile,
                ...styles.courseCard,
                borderLeft: `4px solid ${initialBorderLeftColor}`, // Set initial border transparent
                cursor: 'pointer',
                position: 'relative',
            ...
            onMouseLeave={(e) => {
                e.currentTarget.style.borderLeftColor = initialBorderLeftColor;
        ...
        >
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: '14px', color: '#ffffff', marginTop: '16px' }}>
                ...
                {/* ADDED: Start Again Button (Only for completed courses) */}
                {!isActive && (
                    <button
                        onClick={(e) => {
                            e.stopPropagation();
                            e.preventDefault();
                            onMarkActive(course.id);
                        }}
                        style={{
                            padding: '8px 12px',
                            borderRadius: '8px',
                            backgroundColor: '#4b5563', // A neutral grey/blue for re-activating
                            color: 'white',
                            border: 'none',
                            fontWeight: '600',
                            cursor: 'pointer',
...
export default function DashboardPage() {
    ...
    /**
     * Handles marking a course as completed.
     */
                        ...
                        <div style={{
                            ...
                            {activeCourses.map((c) => (
                                <CourseCard
                                    ...
                                    onMarkActive={() => {}} // Active courses don't need this button
                                />
                            ))}
                        </div>
                    )}
                        ...
                        <div style={{
                            ...
                            {completedCourses.map((c) => (
                                <CourseCard
                                    ...
                                    isActive={false}
                                    onDelete={() => setCourseToDelete(c)}
                                    onMarkComplete={() => {}} // Completed courses don't need the mark complete button
                                    onMarkActive={handleMarkActive} // ADDED: Handle restarting the course
                                />
                            ))}
                        </div>
                    )}
